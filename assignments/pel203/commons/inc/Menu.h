...
namespace pel203 {
	...
	namespace commons {
		...
		public:
			 ...
			 */
			PUBLIC static PMENU_OPTION pickMenuOption(IN MENU_OPTION menu[], IN std::string title = "Lista de comandos disponiveis") {

			    PMENU_OPTION pCommand = NULL;

			    pel203::commons::Logger::log("\n");
				pel203::commons::Logger::log("%s\n", title.c_str());

			    int iListIdx = 0;
			    while (menu[iListIdx].pszOptionName) {
			    	pel203::commons::Logger::log("%2.0d - %s\n", iListIdx + 1, menu[iListIdx].pszOptionName);
			        iListIdx++;
			    }

				do {

					pel203::commons::Logger::log("Escolha o comando [1-%d]: ", iListIdx);
